...
/**
 * Same overlap check as hasTimetableConflict(), but for the recurring
 * weekly Timetable Requirements/Slots workflow, which is keyed by
 * day-of-week rather than a specific calendar date.
 */
function hasWeeklyTimetableConflict($instructorId, $dayOfWeek, $startTime, $endTime) {
    global $pdo;
...
/**
 * Recalculate a timetable_requirements row's status from how many
 * timetable_slots are currently linked to it, vs required_instructors.
 */
function refreshRequirementStatus($requirementId) {
    ...
    $pdo->prepare("UPDATE timetable_requirements SET status = ? WHERE id = ?")
        ->execute([$status, $requirementId]);
}

/**
 * Try to auto-fill any remaining open seats on a timetable_requirements
 * row with eligible active instructors (same rules as getSmartSuggestions:
 * active, not on leave that's relevant, no weekly timetable conflict,
 * prefer matching academic stream, sorted by lowest current workload).
 * Returns the number of seats filled.
 */
function autoAssignTimetableRequirement($requirementId) {
    ...
    $remaining = (int)$requirement['required_instructors'] - $alreadyFilled;
    if ($remaining <= 0) {
        return 0;
    }

    $existingStmt = $pdo->prepare("SELECT instructor_id FROM timetable_slots WHERE requirement_id = ?");
    $existingStmt->execute([$requirementId]);
    $existingIds = array_column($existingStmt->fetchAll(), 'instructor_id');
    ...
    $sql = "
        SELECT i.id,
               (SELECT COALESCE(SUM(ta.duration_hours), 0)
                ...
                WHERE ta.instructor_id = i.id
                  AND ta.is_presentation_panel = 0
                  AND ta.scheduled_date BETWEEN DATE_SUB(CURDATE(), INTERVAL 30 DAY) AND CURDATE()
               ) as recent_workload
        FROM instructors i
        WHERE i.status = 'active'
    ";
    ...
    foreach ($candidates as $candidate) {
        ...
        if (hasWeeklyTimetableConflict($candidate['id'], $requirement['day_of_week'], $requirement['start_time'], $requirement['end_time'])) {
            continue;
        }
        ...
        $pdo->prepare("
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 1, NULL)
        ")->execute([
            $candidate['id'], $requirementId, $requirement['day_of_week'], $requirement['start_time'], $requirement['end_time'],
