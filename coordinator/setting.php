<?php
/**
 * Instructor Coordinator - Profile Settings
 * Smart Instructor Coordination and Workload Management System
 *
 * Mirrors instructor/setting.php, but for a coordinator account: there
 * is no instructors row to join against, so this only manages the
 * users table (name/phone/avatar) and password.
 */
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_profile'])) {
    ...
    if ($fullName === '') {
        ...
        if (empty($error)) {
            ...
            header('Location: ' . app_url('coordinator/setting.php'));
            exit;
        }
    }
}
...
// Load profile (users + role only — a coordinator has no instructors row)
$stmt = $pdo->prepare("
    SELECT u.full_name, u.username, u.email, u.phone, u.avatar_url, u.status,
           r.role_name
    FROM users u
...
.avatar-preview .avatar img {
    ...
    object-fit: cover !important;
            ...
            <div class="row g-4">
                <div class="col-md-6">
                    <div class="card">
                        <div class="card-header"><h5>Account (Read-only)</h5></div>
                        <div class="card-body">
                            <p class="small mb-1"><strong>Role:</strong> <?= htmlspecialchars($profile['role_name'] ?? 'Instructor Coordinator') ?></p>
